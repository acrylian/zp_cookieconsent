...
class zpCookieconsent {
	...
	function getOptionsSupported() {
		$options = array(
			gettext_pl('Message','zpCookieconsent') => array(
			...
			gettext_pl('Dismiss','zpCookieconsent') => array(
			...
			gettext_pl('Learn more','zpCookieconsent') => array(
			...
			gettext_pl('Learn more Link','zpCookieconsent') => array(
				'key' => 'zpcookieconsent_link', 
				'type' => OPTION_TYPE_TEXTBOX,
				'desc' => gettext_pl('Link to your cookie policy / privacy info page.','zpCookieconsent')),
			gettext_pl('Path','zpCookieconsent') => array(
				'key' => 'zpcookieconsent_path', 
				'type' => OPTION_TYPE_TEXTBOX,
				'desc' => gettext_pl('The path for the consent cookie that Cookie Consent uses, to remember that users have consented to cookies. Use to limit consent to a specific path within your website. If empty the default <code>/</code> is used.','zpCookieconsent')),
			gettext_pl('Domain','zpCookieconsent') => array(
			...
			gettext_pl('Expire','zpCookieconsent') => array(
				...
				'type' => OPTION_TYPE_TEXTBOX,
				'desc' => gettext_pl('The number of days Cookie Consent should store the user’s consent information for.','zpCookieconsent')),
			gettext('Theme') => array(
            ...
            'desc' => gettext('The theme you wish to use. Select NONE to use your own css via your custom theme for example.'))
		);
		return $options;
	}
	...
	static function getJS() {
		$message = gettext_pl('This website uses cookies. By continuing to browse the site, you agree to our use of cookies.','zpCookieconsent');
		if(getOption('zpcookieconsent_message')) {
			$message = get_language_string(getOption('zpcookieconsent_message'));
		} 
		$dismiss = gettext_pl('Agree','zpCookieconsent');
		if(getOption('zpcookieconsent_dismiss')) {
			$dismiss = get_language_string(getOption('zpcookieconsent_dismiss'));
		} 
		$learnmore = gettext_pl('More info','zpCookieconsent');
    ...
    window.cookieconsent_options = {
		...
		<script src="<?php echo FULLWEBPATH.'/'.USER_PLUGIN_FOLDER; ?>/zp_cookieconsent/cookieconsent.js"></script>
		<?php
	}
} // class end
?>